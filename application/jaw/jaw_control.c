...
void messageToJawTask(struct JawFlagType source, uint16_t signalType, uint8_t axes, uint16_t* value)
{
    ...
    switch(signalType)
    {
        ...
        case PLAN_DATA:
            if(axes == XY)
            {
                source.cmdPos[X] = value[0]; // value[0] X 位置
                source.cmdPos[Y] = value[1]; // value[1] Y 位置
                source.cmdTime_ms  = value[2];  // <--- 新增：value[2] 时间
                source.stop_flag = value[3];  // <--- 新增：value[3] 停止标志
            }
            else 
            {
                source.cmdPos[axes] = value[0];
                source.cmdTime_ms = value[1]; // 如果是单轴，value[1] 传时�?
            }
        break;
    ...
    case SERVO:
        if(pFlag->cmdPos[axes] > 0)
        {
          ...
          //  if(pFlag->cmdPos[axes] == Y) LOG_I("servo %d\r\n", pFlag->cmdPos[axes]);
            ...
            memcpy(&recvMsg, pFlag, sizeof(struct JawFlagType));
            recvMsg.cmdPos[axes] = (uint32_t)posInPlan * 4 / 2.5;

            // LOG_I("axes: %d, cmdPos: %d, cmdTime_ms: %d, stop_flag: %d\r\n", axes, recvMsg.cmdPos[axes], recvMsg.cmdTime_ms, recvMsg.stop_flag);

            Update_RI_Target(&recvMsg, axes);
            motorEnable(axes);
            motor_servo_start(axes);

#if 0
            double diff = jawControlByAxes[axes].fSVG.TargetPosition - rtFeedback.jawRTPos[axes]/ENCODER_CNT_PER_MM;
            jawControlByAxes[axes].fSVG.TargetPosition = (double)posInPlan/(ENCODER_CNT_PER_MM*2.5);//posInPlan*2.5 before send for precision   /* TargetPosition：单位为mm */
