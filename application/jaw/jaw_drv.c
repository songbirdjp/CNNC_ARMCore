...
static void startPWMOutput(uint8_t axesType)
{
    // if(axesType == X){
...
static int8_t (*motor_stop_callback)(uint8_t axesType) = NULL, (*motor_move_callback)(uint8_t axesType) = NULL;

int8_t motor_callback_register(uint8_t type, void (*callback)(uint8_t axesType))
{
    if (type == 0)
    {
        motor_move_callback = callback;
    }
    else if (type == 1)
    {
...
{
    if (motor_move_callback != NULL)
    {
        motor_move_callback(axesType);
    }
}
...
void motorDisable(uint8_t axesType)
{
    ...
    if (motor_stop_callback != NULL)
    {
        motor_stop_callback(axesType);
    }
}
...
static int8_t motor_pwm_set(uint8_t argc, uint8_t **argv)
{
    ...
    motorCtrlByPWM(10.0, X);
    osDelay(1000);
    motorCtrlByPWM(0.0, X);
    motorDisable(X);
    LOG_I("axis %s motor_pwm_set: %d \r\n", argv[1], atoi(argv[2]));
    return 0;
}
