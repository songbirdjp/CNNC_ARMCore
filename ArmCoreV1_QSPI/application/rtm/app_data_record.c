...
static module_current_status_t rtm_on_current_status = {0};
static module_current_status_t icm_current_status = {0};
...
static struct
{
    uint8_t require_state;
    uint8_t require_ctrl_mode;
    uint16_t rtm_on_plc_info;
    uint32_t interlock_override;
    uint32_t unready_override;
} __attribute__((aligned(1), packed)) rtm_on_state_require = {0};
static struct
{
    uint8_t require_state;
    uint8_t require_ctrl_mode;
    uint16_t ct_status;
    float gantry_velocity_prepare;
    uint32_t interlock_override;
    uint32_t unready_override;
} __attribute__((aligned(1), packed)) icm_state_require = {0};
static module_state_require_t bgm_state_require = {0};
static module_state_require_t qam_state_require = {0};

#define RTM_ON_ID_FILTER (RTM_ON_PLC_ID)
#define ICM_ID_FILTER (ICM_ID)
#define BGM_ID_FILTER (BGM_ID)
#define QAM_ID_FILTER (QAM_ID)
static int32_t app_data_record_filter(uint32_t ID, void *data, uint32_t len)
{
    ...
    switch (ID)
    {
    case RTM_ON_ID_FILTER:
    {
        switch (cmd)
        {
        case INPUT_RTM_ON_ARM_CURRENT_STATE_CMD:
            if (memcmp(&rtm_on_current_status, payload->data + 1, sizeof(module_current_status_t)) == 0)
            {
                goto ret;
            }
            memcpy(&rtm_on_current_status, payload->data + 1, sizeof(module_current_status_t));
            break;
        case OUTPUT_RTM_ON_ARM_REQUIRE_STATE_CMD:
            if (memcmp(&rtm_on_state_require, payload->data + 1, sizeof(module_state_require_t) + sizeof(uint16_t)) == 0)
            {
                goto ret;
            }
    ...
    case ICM_ID_FILTER:
    {
        ...
        case INPUT_ICM_CURRENT_STATE_CMD:
            if (memcmp(&icm_current_status, payload->data + 1, sizeof(module_current_status_t)) == 0)
            {
                goto ret;
            }
            memcpy(&icm_current_status, payload->data + 1, sizeof(module_current_status_t));
            break;
        case OUTPUT_ICM_REQUIRE_STATE_CMD:
            if (memcmp(&icm_state_require, payload->data + 1, sizeof(module_state_require_t)) == 0)
            {
                goto ret;
            }
            memcpy(&icm_state_require, payload->data + 1, sizeof(module_state_require_t));
            break;
        default:
            goto ret;
            break;
    ...
    case BGM_ID_FILTER:
    {
        ...
        case INPUT_BGM_CURRENT_STATE_CMD:
            if (memcmp(&bgm_current_status, payload->data + 1, sizeof(module_current_status_t)) == 0)
            {
                goto ret;
            }
            memcpy(&bgm_current_status, payload->data + 1, sizeof(module_current_status_t));
            break;
        case OUTPUT_BGM_REQUIRE_STATE_CMD:
            if (memcmp(&bgm_state_require, payload->data + 1, sizeof(module_state_require_t)) == 0)
            {
                goto ret;
            }
            memcpy(&bgm_state_require, payload->data + 1, sizeof(module_state_require_t));
            break;
        default:
            goto ret;
            break;
    ...
    case QAM_ID_FILTER:
    {
        ...
        case INPUT_QAM_CURRENT_STATE_CMD:
            if (memcmp(&qam_current_status, payload->data + 1, sizeof(module_current_status_t)) == 0)
            {
                goto ret;
            }
            memcpy(&qam_current_status, payload->data + 1, sizeof(module_current_status_t));
            break;
        case OUTPUT_QAM_REQUIRE_STATE_CMD:
            if (memcmp(&qam_state_require, payload->data + 1, sizeof(module_state_require_t)) == 0)
            {
                goto ret;
            }
            memcpy(&qam_state_require, payload->data + 1, sizeof(module_state_require_t));
            break;
        default:
            goto ret;
            break;
