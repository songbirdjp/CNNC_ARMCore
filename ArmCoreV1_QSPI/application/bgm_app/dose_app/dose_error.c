...
int8_t dose_para_check(enum dose_para_type para_type, enum uart_id id, uint16_t idx, void *value)
{
    ...
    if (value == NULL)
    {
        LOG_E("[%d]: dose parameter value is NULL\r\n", id);
        return -2;
    }
    ...
    case DOSE_PARA_ADC:
        {
            uint32_t adc_value = id == BGM_UART_DOSE1 ? info.cali_dose1_adc[0] : info.cali_dose2_adc[0];
            uint32_t adc_value_1 = id == BGM_UART_DOSE1 ? info.cali_dose1_adc[1] : info.cali_dose2_adc[1];
            if (*(uint32_t *)value != adc_value || *((uint32_t *)value + 1) != adc_value_1)
            {
                LOG_E("[%d]: dose adc value mismatch: %d %d, expected: %d %d\r\n", id, *(uint32_t *)value, *((uint32_t *)value + 1), adc_value, adc_value_1);
    ...
    case DOSE_PARA_DAC:
        {
            uint32_t dac_value = id == BGM_UART_DOSE1 ? info.cali_dose1_dac[0] : info.cali_dose2_dac[0];
            uint32_t dac_value_1 = id == BGM_UART_DOSE1 ? info.cali_dose1_dac[1] : info.cali_dose2_dac[1];
            if (*(uint32_t *)value != dac_value || *((uint32_t *)value + 1) != dac_value_1)
            {
                LOG_E("[%d]: dose dac value mismatch: %d %d, expected: %d %d\r\n", id, *(uint32_t *)value, *((uint32_t *)value + 1), dac_value, dac_value_1);
    ...
    case DOSE_PARA_TRIGGER_INTERVAL_MIN:
        ...
        {
            LOG_E("[%d]: dose trigger interval mismatch: %d, expected: %d\r\n", id, *(uint16_t *)value, info.trigger_interval_min);
    ...
    case DOSE_PARA_DOSE_MODE:
        ...
        {
            LOG_E("[%d]: dose mode mismatch: %d, expected: %d\r\n", id, *(uint8_t *)value, info.dose_mode);
    ...
    case DOSE_PARA_PULSE_MODE:
        ...
        {
            LOG_E("[%d]: dose pulse mode mismatch: %d, expected: %d\r\n", id, *(uint8_t *)value, info.pulse_mode);
    ...
    case DOSE_PARA_PRF:
        ...
        {
            LOG_E("[%d]: dose PRF mismatch: %d, expected: %d\r\n", id, *(uint8_t *)value, info.cali_prf);
    ...
    case DOSE_PARA_DOSE_METER:
        ...
        {
            LOG_E("[%d]: dose meter value mismatch: %f, expected: %f\r\n", id, *(float *)value, info.dose_meter);
    ...
    case DOSE_PARA_CP_NUM:
        {
            ...
            {
                LOG_E("[%d]: dose cp num mismatch: %d, expected: %d\r\n", id, *(uint16_t *)value, beam_obj.info->CPQuantityInBeam);
    ...
    case DOSE_PARA_RI_NUM:
        {
            ...
            {
                LOG_E("[%d]: dose ri num mismatch: %d, expected: %d\r\n", id, *(uint16_t *)value, beam_obj.info->RIQuantityInBeam);
    ...
    case DOSE_PARA_CP_TOLERATE:
        ...
        {
            LOG_E("[%d]: dose cp tolerate mismatch: %d, expected: %d\r\n", id, *(uint8_t *)value, info.cp_tolerate);
    ...
    case DOSE_PARA_CP_RI_MAP:
        {
            ...
            else if (dose_obj->cp_idx_pre + 1 != idx)
            {
                LOG_E("[%d]: dose cp index not continuous: %d, pre: %d\r\n", id, idx, dose_obj->cp_idx_pre);
                dose_obj->error_code.bits.cp_ri_map = 1;
            }
            else if (*(uint16_t *)value != beam_obj.cp_ri_map[idx - 1])
            {
                LOG_E("[%d]: dose cp ri map not match: %d, expected: %d\r\n", id, *(uint16_t *)value, beam_obj.cp_ri_map[idx - 1]);
                dose_obj->error_code.bits.cp_ri_map = 1;
            }
    ...
    case DOSE_PARA_RI_INFO:
        {
            ...
            else if (dose_obj->ri_idx_pre + 1 != idx)
            {
                LOG_E("[%d]: dose ri index not continuous: %d, pre: %d\r\n", id, idx, dose_obj->ri_idx_pre);
            ...
            else
            {
                ...
                {
                    LOG_E("[%d]: dose ri info not match: %f, %f, %f, expect: %f, %f, %f\r\n", id, *tmp, *(tmp + 1), *(tmp + 2), 
    ...
    case DOSE_PARA_BEAM_INFO:
        {
            ...
            {
                LOG_E("[%d]: dose beam info not match: %d, %d, %d, expected: %d, %d, %d\r\n", id, *tmp, *(tmp + 1), *(tmp + 2), 
    ...
    case DOSE_PARA_INTERLOCK_OVERRIDE:
        ...
        {
            LOG_E("[%d]: dose interlock override mismatch: %d, expected: %d\r\n", id, *(uint32_t *)value, info.interlock_override);
    ...
    case DOSE_PARA_TIMESTAMP:
        ...
        {
            LOG_E("[%d]: dose timestamp mismatch: %llu, expected: %llu\r\n", id, *(uint64_t *)value, timestamp_ns_get());
            dose_obj->error_code.bits.timestamp = 1;
        }
        break;
    default:
        LOG_E("[%d]: invalid dose parameter type: %d\r\n", id, para_type);
        ret = -3;
        break;
    }
