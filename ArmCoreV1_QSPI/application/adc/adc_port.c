...
/* 
 * adc1: convert time = (sample time + 0.5 + Nbit / 2) * fclk;  1 channel total time = 17.06us
 *       rank 1: channel 4,    rank 2: channel 7,    rank 3: channel 8
 * 
 * adc3: convert time = (sample time + 0.5 + Nbit) * fclk;      3 channel total time = 40.81us
...
int8_t adc_sample_start(void)
{
    ...
    status = HAL_LPTIM_Counter_Start(&hlptim1, period);
    if (status != HAL_OK)
    {
