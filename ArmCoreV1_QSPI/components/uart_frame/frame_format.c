...
wait_recv:
    ...
    if (self->crc_check_state == true)
    {
        ...
        {
            LOG_E("recv crc check error: %#.8x %#.8x\r\n", crc, *(uint32_t *)&(self->rx_buffer[recv_len - FRAME_CRC_LEN]));
...
#if 0
    LOG_I("recv %d bytes data: ", *data_len);
