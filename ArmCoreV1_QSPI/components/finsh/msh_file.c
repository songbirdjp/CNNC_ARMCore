...
static int cmd_mount(int argc, char **argv)
{
    if (argc == 1)
    {
        ...
        printf("filesystem  device  mountpoint\n");
        printf("----------  ------  ----------\n");
    ...
    else if (argc == 4)
    {
        ...
        printf("mount device %s(%s) onto %s ... ", device, fstype, path);
    ...
    else if (argc == 3)
    {
        ...
        /* mount a filesystem to the specified directory */
        printf("mount (%s) onto %s ... ", fstype, path);
...
static int cmd_umount(int argc, char **argv)
{
    ...
    if (argc != 2)
    {
        printf("Usage: unmount <mountpoint>.\n");
        return -1;
    }

    printf("unmount %s ... ", path);
...
static int cmd_tail(int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        printf("Usage: tail [-n numbers] <filename>\n");
        return -1;
    }
    else if (argc == 2)
    {
