                                     ...
                                     void *const arg)
{
    ...
    {
        device->irq_handle_callback[cmd - PIN_CMD_SET_IRQ_HANDLE_RISING_CALLBACK_HIGH] = (irq_handle_callback_t)arg;
        break;
    }
    default:
    {
                         ...
                         void *const user_data)
{
    ...
    for (uint8_t i = 0; i < CALLBACK_TIRE_LIMIT; i++)
    {
        self->irq_handle_callback[i] = NULL;
    }
...
void device_pin_rising_irq_handel(device_pin_t *const self)
{
    ...
    {
        if (self->irq_handle_callback[i] != NULL)
        {
            self->irq_handle_callback[i]((void *)self);
        }
    }
}
