...
static int8_t drv_uart_init(uart_dev_t *const self)
{
    ...
    else if (!memcmp(uart_drv->dev.name, UART_DEV_NAME_UART9, sizeof(UART_DEV_NAME_UART9)))
    {
        MX_UART9_Init();
    }
    else
...
static int32_t drv_uart9_init(void)
{
    int32_t ret = drv_uart_register(uart_drv_get(&huart9),
                                    &huart9,
                                    UART_DEV_NAME_UART9,
                                    UART_TYPE_FULL_DUPLEX);
    if (ret != 0)
...
INIT_DEVICE_EXPORT(drv_uart9_init);
