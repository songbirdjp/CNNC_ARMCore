...
static LAN9252_APPL_OPS lan9252_app_ops = {0};


static void appl_output_update(UINT16 *pData)
{
    UINT16 j = 0;
    UINT16 *pTmpData = (UINT16 *) pData;
...
int8_t lan9252_app_ops_init(osEventFlagsId_t output_event, uint32_t event_flag, void (*appl_cb)(void))
{
    ...
    lan9252_app_ops.pdo_output_queue = osMessageQueueNew (3, MAX_PD_OUTPUT_SIZE, NULL);
    lan9252_app_ops.pdo_output_update_mutex = osMutexNew(&mutex_attributes);

    mutex_attributes.name = "pdo_input_mutex";
    lan9252_app_ops.pdo_input_update_mutex = osMutexNew(&mutex_attributes);
